...
export default function Vacaciones() {
  ...
  const cargar = async () => {
    ...
    try {
      const q = esAdmin
        ...
        : query(collection(db,"vacaciones"), where("empleadoId","==",user.uid), orderBy("creadaEn","desc"));
      const [vSnap, eSnap, uSnap] = await Promise.all([
        getDocs(q),
        getDocs(collection(db,"empresas")),
        esAdmin ? getDocs(collection(db,"usuarios")) : Promise.resolve({docs:[]}),
      ]);
      setSolicitudes(vSnap.docs.map(d=>({id:d.id,...d.data()})));
  ...
  const guardar = async () => {
    ...
    if (dias <= 0) { showToast("La fecha de fin debe ser posterior al inicio","error"); return; }
    setGuardando(true);
    try {
      const datos = {
    ...
    <div>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
        <div>
          ...
          {pendientes > 0 && esAdmin && (
            <span style={{ fontSize:13, color:"#BA7517" }}>⚠ {pendientes} solicitud{pendientes>1?"es":""} pendiente{pendientes>1?"s":""}</span>
          )}
        </div>
        ...
        <table className="tabla">
          <thead>
            <tr>
              ...
              <th>Desde</th><th>Hasta</th><th>Días</th><th>Estado</th><th>Acciones</th>
            </tr>
          </thead>
          <tbody>
            {lista.length === 0 && (
