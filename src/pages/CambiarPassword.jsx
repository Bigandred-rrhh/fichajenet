...
export default function CambiarPassword() {
  ...
  return (
    <div style={{ maxWidth:440 }}>
      {ToastUI}
      <h1 style={{ fontSize:22, fontWeight:700, marginBottom:20 }}>Cambiar contraseña</h1>
      <div className="card">
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label className="form-label">Contraseña actual</label>
            <input className="form-input" type="password" value={actual}
              onChange={e=>setActual(e.target.value)} required placeholder="Tu contraseña actual" />
          </div>
          <div className="form-group">
            <label className="form-label">Nueva contraseña</label>
            <input className="form-input" type="password" value={nueva}
              onChange={e=>setNueva(e.target.value)} required placeholder="Mínimo 6 caracteres" />
          </div>
          <div className="form-group">
            <label className="form-label">Repetir nueva contraseña</label>
            <input className="form-input" type="password" value={repetir}
              onChange={e=>setRepetir(e.target.value)} required placeholder="Repite la nueva contraseña" />
          </div>
      ...
      <div style={{ marginTop:16, padding:"12px 16px", background:"#FFF3CD",
        ...
        ⚠ Tras cambiar la contraseña tendrás que volver a iniciar sesión en todos tus dispositivos.
      </div>
    </div>
  );
}
