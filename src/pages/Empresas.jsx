  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
        <h1 style={{ fontSize:22, fontWeight:700 }}>Empresas S.L.</h1>
        <button className="btn btn-primary" onClick={() => abrir(null)}>+ Nueva empresa</button>
      </div>

      <div className="card">
        <table className="tabla">
          <thead>
            <tr><th>Nombre</th><th>CIF</th><th>Convenio</th><th>Contacto RRHH</th><th>Acciones</th></tr>
          </thead>
          <tbody>
            {empresas.length === 0 && (
      ...
      {modal && (
        <div className="modal-overlay" onClick={() => setModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-title">{editId ? "Editar empresa" : "Nueva empresa S.L."}</div>
            {[
              { key:"nombre",    label:"Nombre completo *",   ph:"Distribuciones López S.L." },
              { key:"cif",       label:"CIF *",                ph:"B-12345678" },
              { key:"domicilio", label:"Domicilio social",     ph:"Calle Gran Vía 45, Madrid" },
              { key:"convenio",  label:"Convenio colectivo",   ph:"Comercio, Hostelería..." },
              { key:"contacto",  label:"Email responsable RRHH", ph:"rrhh@empresa.com" },
            ].map(f => (
